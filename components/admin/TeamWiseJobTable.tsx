...
import { TeamWiseData } from '../../types';

interface TeamWiseJobTableProps {
  data: TeamWiseData[];
}

const TeamWiseJobTable: React.FC<TeamWiseJobTableProps> = ({ data }) => {
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full">
        <thead className="bg-gray-100">
            <tr>
              <th className={thClass}>Recruiter Name</th>
        ...
        <tbody className="bg-white divide-y divide-gray-100">
            {data.length > 0 ? data.map((item, index) => (
              <tr key={index}>
                <td className={`${tdClass} font-bold text-gray-900`}>{item.recruiterName}</td>
              ...
              <tr>
                <td colSpan={9} className="px-6 py-10 text-center text-gray-500">
                  No team-wise job data available.
                </td>
              </tr>
            )}
        </tbody>
...
export default TeamWiseJobTable;
